...
import style from './ScoreAndComment.scss'
import DarkBox from 'components/DarkBox'
import scoretitle from 'assets/scoretitle.png'
import {api} from 'common/app'
  
export class ScoreAndComment extends Component {
constructor(props) {
  super(props);
  this.state = {
      id:null,
...
refreshProps(props) {
    ...
    this.state.content = props.content?props.content:this.state.content;
...
ConfirmResult(){
    if (this.state.score&&this.state.comment) {
        if (this.state.id) {
            api.setCaseScoreContentMobile(this.state.id,this.state.score,this.state.comment).then(res=>{
                if (res.code === 200) {
...
render() {
  return (
    <DarkBox >
        <div className={style.ScoreAndComment}>
            ...
            <div className={style.CommentBox}>
                <textarea className={style.TextArea} value={this.state.comment} onChange={this.HandleComment}></textarea>
            </div>
